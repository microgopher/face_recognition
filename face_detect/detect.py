...
class ObjectDetectorCascadeOpenCV(ObjectDetector):
    def __init__(self, model_name, basename='face', tgtdir='face_detected', min_height_dec=20, min_width_dec=20,
        ...
        self.tgtdir = tgtdir
        face_path = '%s/%s' % (self.tgtdir, 'faces')
        if not os.path.isdir(os.path.join(face_path)):
            os.makedirs(face_path)
        self.basename = basename + '-' + time.strftime("%Y%m%d-%H%M%S")
    ...
    def run(self, img):
        ...
        for i, (x, y, w, h) in enumerate(faces):
            images.append(self.sub_image('%s/%s/%s-%d.jpg' % (self.tgtdir, 'faces', self.basename, i + 1), img, x, y, w, h))
